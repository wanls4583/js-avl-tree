...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__avl_js__ = __webpack_require__(1);


function testInsert() {
    console.log('%ctestInsert', 'color:red');
    var t = Date.now();
    var tree = new __WEBPACK_IMPORTED_MODULE_0__avl_js__["a" /* default */]();
    var size = 100000;
...
function testCompartor() {
    ...
    var tree = new __WEBPACK_IMPORTED_MODULE_0__avl_js__["a" /* default */](compartor);
    var size = 10;
    for (var i = 1; i <= size; i++) {
        var key = { key: i };
...
function AVLTree(compartor, ifMerge) {
    ...
    this.compartor = compartor || function (arg1, arg2) {
        return arg1 - arg2;
    };
...
_proto._insert = function (root, node) {
    if (this.compartor(root.key, node.key) == 0) {
        return false;
    } else if (this.compartor(root.key, node.key) > 0) {
        //插入左子树
        if (root.lChild) {
            //在左子树上递归插入
...
_proto._delete = function (root, key) {
    ...
    if (this.compartor(root.key, key) == 0) {
        if (!root.lChild && !root.rChild) {
            ...
            return root;
        } else if (this._getHeight(root.lChild) > this._getHeight(root.rChild)) {
            //用左子树上最大的节点代替root
...
_proto._checkBalance = function (node) {
    if (node.rChild && this._getHeight(node.rChild) - this._getHeight(node.lChild) == 2) {
        //不平衡，需要调整
        var rc = node.rChild;
        if (this._getHeight(node.rChild.rChild) > this._getHeight(node.rChild.lChild)) {
            //左旋转
            this._lRotate(node);
        } else {
            //先右旋转，再左旋转
    ...
    } else if (node.lChild && this._getHeight(node.lChild) - this._getHeight(node.rChild) == 2) {
        //不平衡，需要调整
        var lc = node.lChild;
        if (this._getHeight(node.lChild.lChild) > this._getHeight(node.lChild.rChild)) {
            //右旋转
            this._rRotate(node);
        } else {
            //先左旋转，再右旋转
...
/* harmony default export */ __webpack_exports__["a"] = (AVLTree);
